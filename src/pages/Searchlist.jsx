...
import MobileSearchlistPage from "./mobile/MobileSearchlist";
                ...
                <div className="flex justify-between items-center mb-6 pb-4 border-b">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            {/* ✅ Vendor Name - Show if available
                            {product.vendor_details?.business_name && (
                                <p className="text-xs text-gray-500 mb-2">
                                    by <span className="font-medium text-gray-700 hover:text-blue-600 cursor-pointer"
                                        onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            navigate(`/vendor/${product.vendor_details.id}`);
                                        }}>
                                        {product.vendor_details.business_name}
                                    </span>
                                </p>
...
// ProductCard Component - Enhanced with Amazon/Flipkart Style Coupon Display
const ProductCard = ({ product, openModal, onAddToCart, showLoginModal, hasCoupons, onViewCoupons }) => {
    ...
    const getImageUrl = (product) => {
        ...
        let image =
            product?.stocks?.[0]?.variant_image ||
            product?.main_image ||
    ...
    return (
        <div className="bg-white p-4 rounded-xl shadow-md border border-gray-200 hover:shadow-xl transition-all duration-300 group cursor-pointer h-full flex flex-col relative">
            ...
            {/* Product Image */}
            <div className={`h-48 flex items-center justify-center mb-3 overflow-hidden rounded-lg bg-gray-50 ${hasCoupons ? 'mt-8' : 'mt-0'}`}>
                <img
                ...
                <div className="flex gap-6">
                        ...
                        <div className="flex gap-2 mt-3">
                            {modalProduct.gallery && modalProduct.gallery.slice(0, 4).map((galleryItem, index) => (
                                <img
                                    ...
                                    alt={`${modalProduct.product_name} - Gallery ${index + 1}`}
                                />
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="w-1/2">
                        ...
                        <div className="flex gap-4 mt-6">
                            ...
                            <button
                                onClick={handleViewDetails}
                                className="flex-1 rounded-lg bg-blue-500 py-2.5 text-white font-semibold hover:bg-blue-600 transition shadow-lg shadow-blue-200 text-sm"
                            >
                                View Details
                            </button>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4">
                        ...
                        <FilterSection title="Condition">
                            <FilterList>
                                ...
                                {["new", "like_new", "refurbished", "used"].map(cond => (
                                        ...
                                        <input
                                            ...
                                            onChange={(e) =>
                                                e.target.checked
                                                    ...
                                                    : setSelectedConditions(
                                                        selectedConditions.filter(v => v !== cond)
                                                    )
                                            }
                                            className="w-4 h-4"
                                        />
...
export default function Searchlist() {
    const [isMobile, serIsMobile] = useState(() => window.innerWidth < 768);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            if (keywords) {
                // Case 1: We have search keywords
                await fetchProducts(keywords);
            } else if (selectedConditions.length > 0 || sortBy) {
                // Case 2: No search, but we have condition filters or sort
                await fetchProductsByCondition();
            } else {
                // Case 3: No search and no filters - show empty state
                setProducts([]);
                setLoading(false);
            }
        };
        ...
        fetchData();
    }, [location.search, selectedConditions, sortBy]);
    useEffect(() => {
    ...
    // ✅ Function to fetch products by condition
    const fetchProductsByCondition = async () => {
        try {
            ...
            console.error("Error fetching products by condition:", err);
    ...
    // Fetch products from API - Updated with vendor search support
    const fetchProducts = async (keyword) => {
        try {
            ...
            // Step 1: Search for vendors matching the keyword
            try {
                const vendorResponse = await publicAxios.get(`/ecommerce/public/vendors/search/?query=${encodeURIComponent(keyword)}`);
                if (vendorResponse.data && Array.isArray(vendorResponse.data)) {
                    vendorIds = vendorResponse.data.map(v => v.id);

                }
            } catch (vendorErr) {
                console.error("Error searching vendors:", vendorErr);
            }

            // Step 2: If we have vendor IDs, fetch products from those vendors
            if (vendorIds.length > 0) {
                try {
                    ...
                    // Filter products by vendor IDs
                    const vendorProducts = allProducts.filter(product => {
                        // Check both product.vendor (direct ID) and product.vendor_details?.id
                        return vendorIds.includes(product.vendor) ||
                            (product.vendor_details && vendorIds.includes(product.vendor_details.id));
                    });


                    productsArray = vendorProducts;
                } catch (err) {
                    console.error("Error fetching vendor products:", err);
                }
            }
            // Step 3: If no vendors found or still want product search, do keyword search
            else {
                try {
                    const productUrl = `/ecommerce/public/search-product/?keywords=${encodeURIComponent(keyword)}`;
                    const productResponse = await publicAxios.get(productUrl);
            ...
            // Step 4: Format all products with proper image URLs
            const formattedProducts = productsArray.map(product => {
                ...
                const vendorDetails = product.vendor_details || {};
                ...
                return {
                    ...
                    vendor_details: {
                        ...
                        business_name: vendorDetails.business_name || "Unknown Vendor",
                        store_logo: fixImageUrl(vendorDetails.store_logo)
                    },
                    ...
                    max_quantity: stock?.maximum_order_quantity || 10,
    ...
    useEffect(() => {
        ...
        if (conditions.includes(keywords.toLowerCase())) {
            setSelectedConditions([keywords]);
        }

    }, [location.search]);

    // Filter products based on search, price range, categories, brands, and conditions
    const filteredProducts = useMemo(() => {
        if (products.length === 0) {
            return [];
        }

        // Check if this is a vendor search (all products from same vendor might have same vendor ID)
        const isVendorSearch = products.length > 0 &&
            products.every(p => p.vendor === products[0].vendor);

        return products.filter(p => {
            // For vendor search, don't filter by product name
            // For regular search, filter by product name
            const conditions = ["new", "used", "refurbished", "like new"];

            const isConditionSearch = conditions.includes(searchTerm.toLowerCase());

            const matchesSearch =
                isVendorSearch || isConditionSearch
                    ? true
                    : (
                        !searchTerm ||
            ...
            const productPrice = Number(p.stocks?.[0]?.selling_price || 0);
            ...
            // Category filter
            const productCategoryId = p.category || p.category_details?.id;
            const matchesCategory = selectedCategories.length === 0 ||
                (productCategoryId && selectedCategories.includes(productCategoryId));

            // Brand filter
            const productBrandId = p.brand || p.brand_details?.id;
            const matchesBrand = selectedBrands.length === 0 ||
                (productBrandId && selectedBrands.includes(productBrandId));

            // ✅ Product Condition filter
            const productCondition = (p.product_condition || "New").toLowerCase();

            const matchesCondition =
                selectedConditions.length === 0 ||
                selectedConditions
                    .map(c => c.toLowerCase())
                    .includes(productCondition);

            const result = matchesSearch && matchesPrice && matchesCategory && matchesBrand && matchesCondition;

            return result;
        });
    }, [products, searchTerm, minPrice, maxPrice, selectedCategories, selectedBrands, selectedConditions]); // Add selectedConditions to dependencies

    // Debug log for filtered products
    useEffect(() => {

    }, [filteredProducts]);
    if (isMobile) return <MobileSearchlistPage />;
                                ...
                                {['New', 'Used', 'Refurbished', 'Like new'].map(cond => (
                                    <li key={cond} className="flex items-center gap-2">
                                        <input
                                            ...
                                            onChange={(e) =>
                                                e.target.checked
                                                    ? setSelectedConditions([...selectedConditions, cond.toLowerCase()])
                                                    : setSelectedConditions(
                                                        selectedConditions.filter(v => v !== cond.toLowerCase())
                                                    )
                                            }
                                            className="w-4 h-4"
                                        />
                        ...
                        {(selectedCategories.length > 0 || selectedBrands.length > 0 || selectedConditions.length > 0) && (
                            <div className="mb-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <button
                                        onClick={() => {
                                            setSelectedCategories([]);
                                            setSelectedBrands([]);
                                            setSelectedConditions([]); // Add this
                                        }}
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {/* ✅ Condition filters - Add this section */}
