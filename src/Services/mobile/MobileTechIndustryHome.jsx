// src/pages/mobile/MobileTechIndustryHome.tsx
...
export default function MobileTechIndustryHome() {
        ...
        <section key={`${blockIndex}-${index}`} className="py-5">
          <div className="flex justify-between items-center mb-4 px-1">
            ...
            <button
              ...
              className="text-cyan-600 text-[11px] font-semibold flex items-center gap-1 flex-shrink-0"
              style={{ fontFamily: "'Inter', sans-serif" }}
            >
  ...
  return (
    ...
    <div className="bg-[#f8fafc] min-h-screen pb-20 overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-lg border-b border-gray-100 px-4 py-3 flex items-center justify-between overflow-x-hidden">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-gray-500 truncate">IT services & solutions</p>
          </div>
        </div>
      ...
      <section className="relative mx-3 mt-3 rounded-3xl overflow-hidden h-[250px]">
        <img
          src="https://images.pexels.com/photos/3894153/pexels-photo-3894153.jpeg?auto=compress&cs=tinysrgb&w=800"
          alt="tech-banner"
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-4">
          <h2 className="text-white text-[16px] font-bold leading-snug" style={{ fontFamily: "'Inter', sans-serif" }}>
            Find Best Tech <br />Companies & IT Services
          </h2>
          <p className="text-cyan-200 text-[10px] mt-0.5" style={{ fontFamily: "'Inter', sans-serif" }}>
            <span className="font-bold text-white">300+</span> verified tech companies
          </p>
          ...
          <div className="mt-2.5 bg-white rounded-2xl shadow-xl p-1.5 flex items-center gap-1.5 relative z-10">
            ...
            <div className="flex-[2] min-w-[100px]">
              <input
                type="text"
                placeholder="Search tech companies, IT services..."
                value={searchKeyword}
                onChange={(e) => setSearchKeyword(e.target.value)}
                onKeyPress={(e) => e.key === 'Enter' && navigateToServiceList()}
                className="w-full px-3 py-2 text-[10px] bg-gray-50 rounded-xl outline-none focus:ring-2 focus:ring-cyan-500 transition"
                style={{ fontFamily: "'Inter', sans-serif" }}
              />
            </div>
            ...
            <button
              ...
              className="px-3.5 py-2 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white text-[10px] font-semibold rounded-xl hover:shadow-lg transition flex-shrink-0"
              style={{ fontFamily: "'Inter', sans-serif" }}
            >
              <FiSearch size={14} />
            </button>
        ...
        <div className="relative px-3 -mt-1 z-[9999] overflow-x-hidden">
          <div className="w-full bg-white shadow-2xl rounded-xl max-h-52 overflow-y-auto border border-gray-200">
            ...
            {cities.map((city, idx) => (
              <div
                key={city.id || idx}
                className="px-3 py-2.5 text-[11px] hover:bg-cyan-50 cursor-pointer text-gray-700 transition flex items-center justify-between"
                onClick={() => {
      ...
      <div className="px-3 py-4 overflow-x-hidden">
        ...
        <section className="grid grid-cols-3 gap-2.5 mb-5">
          {[
            { icon: <FaCubes className="text-xl text-cyan-600" />, title: "Scalable", count: "100+" },
            { icon: <FaHeadset className="text-xl text-green-600" />, title: "Support", count: "24/7" },
            { icon: <FaLightbulb className="text-xl text-pink-600" />, title: "Innovation", count: "50+" },
          ].map((item, i) => (
            <div
              key={i}
              onClick={navigateToServiceList}
          ...
          <section className="py-3">
            <div className="relative rounded-2xl overflow-hidden shadow-sm h-36 group cursor-pointer overflow-x-hidden" onClick={() => window.open(bigAd.url, "_blank")}>
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3.5">
                ...
                <button className="inline-block mt-1 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white text-[10px] font-medium px-3.5 py-0.5 rounded-full hover:shadow-lg transition">
                  Shop Now →
                </button>
              </div>
            </div>
      ...
      <div className="p-3">
        ...
        <div className="flex gap-1.5 mt-2">
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white text-[9px] font-medium py-1.5 rounded-lg hover:shadow-md transition"
            style={{ fontFamily: "'Inter', sans-serif" }}
          >
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 w-full">
              ...
              <div className="mt-2 px-1 w-full">
                {field.type === 'textarea' ? (
                  <textarea
                    ...
                    rows="3"
                    className="w-full px-4 py-3 text-[13px] border-2 border-cyan-500 rounded-xl outline-none resize-none focus:ring-2 focus:ring-cyan-300 transition"
                    style={{ fontFamily: "'Inter', sans-serif" }}
                    autoFocus
                  />
                ) : (
                  <input
                    ...
                    value={value}
                    onChange={handleChange}
                    className="w-full px-4 py-3 text-[13px] border-2 border-cyan-500 rounded-xl outline-none focus:ring-2 focus:ring-cyan-300 transition"
                    style={{ fontFamily: "'Inter', sans-serif" }}
                    autoFocus
                  />
                )}
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full py-3.5 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white rounded-xl font-semibold text-[13px] disabled:opacity-50 hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 mt-4"
        style={{ fontFamily: "'Inter', sans-serif" }}
      >
        {submitting ? (
          <>
