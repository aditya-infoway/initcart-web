// src/pages/mobile/MobileProfessionalDetail.tsx
...
import { FaWhatsapp, FaStar, FaMapMarkerAlt, FaUserTie, FaBuilding, FaTimes } from "react-icons/fa";
import { publicAxios } from "../../api/axios";
import MobileServiceReviewSection from "./MobileServiceReviewSection";

export default function MobileProfessionalDetail() {
  const { id } = useParams();
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gray-50">
        ...
        <span className="mt-3 text-detail-body text-gray-600">Loading professional details...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gray-50 px-4">
        ...
        <button onClick={() => navigate("/professionalhome")} className="bg-indigo-600 text-white px-6 py-2 rounded-lg text-detail-btn-sm hover:bg-indigo-700">
    ...
    <div className="bg-gray-50 min-h-screen pb-24 overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="relative mx-3 mt-3 rounded-2xl overflow-hidden">
        ...
        {service.subcategory_name && (
          <span className="absolute top-3 left-3 bg-indigo-600 text-white text-detail-xs font-medium px-2.5 py-1 rounded-full">
            {service.subcategory_name}
          </span>
        )}
      </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto px-3 mt-2 scrollbar-hide">
          ...
          {additionalImages.length > 6 && (
            <div className="rounded-lg w-16 h-16 bg-gray-200 flex items-center justify-center flex-shrink-0 cursor-pointer" onClick={() => setIsOpen(true)}>
              <span className="text-detail-xs font-medium text-indigo-600">+{additionalImages.length - 6}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="px-4 mt-3 flex gap-2 overflow-x-hidden">
        {service.contact_no && (
          <a href={`tel:${service.contact_no}`} className="flex-1 bg-indigo-600 text-white text-detail-btn-sm font-medium py-2.5 rounded-xl flex items-center justify-center gap-1.5 hover:bg-indigo-700 transition">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <h2 className="text-detail-subtitle font-bold text-gray-900 mb-2 flex items-center gap-2">
                <FaUserTie className="text-indigo-600 text-detail-icon-sm" /> About
              </h2>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { icon: FiTag, label: "Category", value: service.subcategory_name || "Professional" },
                  { icon: FiBriefcase, label: "Business", value: service.business_name },
          ...
          <div className="space-y-4">
            ...
            {service.location && (
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                ...
                <a href={service.location} target="_blank" rel="noopener noreferrer" className="text-indigo-600 text-detail-text break-all hover:text-indigo-800">
                  {service.location}
                </a>
              </div>
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 z-50 shadow-lg">
        <button
          ...
          className="w-full bg-gradient-to-r from-indigo-600 to-indigo-700 text-white text-detail-btn font-semibold py-3 rounded-xl hover:shadow-lg transition"
        >
          Send Inquiry
        </button>
      </div>
          ...
          <div className="p-5">
            <form onSubmit={handleInquirySubmit} className="space-y-3 w-full">
                      ...
                      <div className="mt-2 px-1 w-full">
                        {field.type === 'textarea' ? (
                          <textarea
                            ...
                            rows="3"
                            className="w-full px-4 py-3 text-detail-input border-2 border-indigo-500 rounded-xl outline-none resize-none focus:ring-2 focus:ring-indigo-300 transition"
                            autoFocus
                          />
                        ) : (
                          <input
                            ...
                            value={value}
                            onChange={(e) => setInquiryData({ ...inquiryData, [field.name]: e.target.value })}
                            className="w-full px-4 py-3 text-detail-input border-2 border-indigo-500 rounded-xl outline-none focus:ring-2 focus:ring-indigo-300 transition"
                            autoFocus
                          />
                        )}
                      </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-xl font-semibold text-detail-btn disabled:opacity-50 hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 mt-2"
              >
                {submitting ? (
                  <>
