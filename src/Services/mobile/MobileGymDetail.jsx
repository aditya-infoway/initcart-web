// src/pages/mobile/MobileGymDetail.tsx
...
import { FaWhatsapp, FaStar, FaMapMarkerAlt, FaDumbbell, FaTimes } from "react-icons/fa";
import { publicAxios } from "../../api/axios";
import MobileServiceReviewSection from "./MobileServiceReviewSection";

export default function MobileGymDetail() {
  const { id } = useParams();
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gray-50">
        ...
        <span className="mt-3 text-detail-body text-gray-600">Loading gym details...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gray-50 px-4">
        ...
        <button onClick={() => navigate("/gymhome")} className="bg-orange-600 text-white px-6 py-2 rounded-lg text-detail-btn-sm hover:bg-orange-700">
          Back to Gyms
        </button>
      </div>
    );
    ...
    <div className="bg-gray-50 min-h-screen pb-24 overflow-x-hidden" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="relative mx-3 mt-3 rounded-2xl overflow-hidden">
        ...
        {service.subcategory_name && (
          <span className="absolute top-3 left-3 bg-orange-600 text-white text-detail-xs font-medium px-2.5 py-1 rounded-full">
            {service.subcategory_name}
          </span>
        )}
      </div>
        ...
        <div className="flex gap-1.5 overflow-x-auto px-3 mt-2 scrollbar-hide">
          ...
          {additionalImages.length > 6 && (
            <div className="rounded-lg w-16 h-16 bg-gray-200 flex items-center justify-center flex-shrink-0 cursor-pointer" onClick={() => setIsOpen(true)}>
              <span className="text-detail-xs font-medium text-orange-600">+{additionalImages.length - 6}</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="px-4 mt-3 flex gap-2 overflow-x-hidden">
        {service.contact_no && (
          <a href={`tel:${service.contact_no}`} className="flex-1 bg-orange-600 text-white text-detail-btn-sm font-medium py-2.5 rounded-xl flex items-center justify-center gap-1.5 hover:bg-orange-700 transition">
        ...
        <div className="flex gap-6 overflow-x-auto scrollbar-hide">
          {["details", "location", "reviews"].map((tab) => (
            <button
              ...
              className={`pb-2.5 text-detail-btn-sm font-medium whitespace-nowrap transition ${activeTab === tab ? "border-b-2 border-orange-600 text-orange-600" : "text-gray-500"
                }`}
            >
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              <h2 className="text-detail-subtitle font-bold text-gray-900 mb-2 flex items-center gap-2">
                <FaDumbbell className="text-orange-600 text-detail-icon-sm" /> About
              </h2>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { icon: FiTag, label: "Category", value: service.subcategory_name || "Gym" },
            ...
            {items.length > 0 && (
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <h2 className="text-detail-subtitle font-bold text-gray-900 mb-3">Services & Packages</h2>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-0.5">
                            <FaDumbbell className="text-orange-500 w-3.5 h-3.5 flex-shrink-0" />
                        ...
                        {formatCurrency(item.price) && (
                          <div className="ml-2 flex-shrink-0">
                            <span className="text-orange-600 font-bold text-detail-price">{formatCurrency(item.price)}</span>
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-4">
            ...
            {service.location && (
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                ...
                <a href={service.location} target="_blank" rel="noopener noreferrer" className="text-orange-600 text-detail-text break-all hover:text-orange-800">
                  {service.location}
                </a>
              </div>
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 z-50 shadow-lg">
        <button
          ...
          className="w-full bg-gradient-to-r from-orange-600 to-orange-700 text-white text-detail-btn font-semibold py-3 rounded-xl hover:shadow-lg transition"
        >
          Send Inquiry
        </button>
      </div>
          ...
          <div className="p-5">
            <form onSubmit={handleInquirySubmit} className="space-y-3 w-full">
                      ...
                      <div className="mt-2 px-1 w-full">
                        {field.type === 'textarea' ? (
                          <textarea
                            ...
                            rows="3"
                            className="w-full px-4 py-3 text-detail-input border-2 border-orange-500 rounded-xl outline-none resize-none focus:ring-2 focus:ring-orange-300 transition"
                            autoFocus
                          />
                        ) : (
                          <input
                            ...
                            value={value}
                            onChange={(e) => setInquiryData({ ...inquiryData, [field.name]: e.target.value })}
                            className="w-full px-4 py-3 text-detail-input border-2 border-orange-500 rounded-xl outline-none focus:ring-2 focus:ring-orange-300 transition"
                            autoFocus
                          />
                        )}
                      </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-3.5 bg-gradient-to-r from-orange-600 to-orange-700 text-white rounded-xl font-semibold text-detail-btn disabled:opacity-50 hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-2 mt-2"
              >
                {submitting ? (
                  <>
