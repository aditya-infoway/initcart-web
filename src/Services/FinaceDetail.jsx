// src/Services/FinanceDetail.tsx
...
import {
  ...
  FiClock, FiGrid, FiLoader, FiTag, FiGlobe, FiMap, FiBriefcase, FiUser, FiDollarSign,
} from "react-icons/fi";
import { FaWhatsapp, FaStar, FaMapMarkerAlt, FaUserTie, FaChartLine, FaShieldAlt } from "react-icons/fa";
import { publicAxios } from "../api/axios";
import MobileFinanceDetail from "./mobile/MobileFinanceDetail";
...
export default function FinanceDetail() {
  ...
  useEffect(() => {
    const fetchServiceDetail = async () => {
      try {
        setLoading(true);
        const response = await publicAxios.get(`/api/public/finance-service/${id}/`);
        if (response.data) {
  ...
  const handleFavorite = () => {
    const favs = JSON.parse(localStorage.getItem("finance_favorites") || "[]");
    if (service && !favs.includes(service.id)) {
      favs.push(service.id);
      localStorage.setItem("finance_favorites", JSON.stringify(favs));
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        ...
        <span className="ml-3 text-lg text-gray-600">Loading financial service details...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center h-screen">
        ...
        <button onClick={() => navigate("/financehome")} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700">
  ...
  // Prepare images
  const mainImage = service.main_image || "https://placehold.co/800x500/2563EB/white?text=Finance+Service";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">{service.business_name || "Financial Service"}</h1>
          {service.subcategory_name && (
            <span className="inline-block mt-2 bg-blue-100 text-blue-800 text-xs px-3 py-1 rounded-full font-medium">
              {service.subcategory_name}
            </span>
          )}
        </div>
        <div className="flex items-center space-x-4 mt-3 md:mt-0">
          ...
          <button onClick={handleShare} className="text-gray-400 hover:text-blue-600 transition">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="relative mb-3">
              ...
              {allImages.length > 0 && (
                <button
                  ...
                  className="absolute bottom-4 right-4 bg-blue-600 text-white font-medium py-2 px-4 rounded-full hover:bg-blue-700 flex items-center text-sm shadow-lg"
                >
              ...
              <div className="grid grid-cols-4 md:grid-cols-5 gap-2">
                ...
                {additionalImages.length > 5 && (
                  <div
                    ...
                    onClick={() => setIsOpen(true)}
                  >
                    <span className="text-blue-600 font-medium">+{additionalImages.length - 5} more</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="space-y-6">
              ...
              <section className="p-6 bg-gray-50 rounded-xl">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <FaUserTie className="text-blue-600" /> About {service.business_name}
                </h2>
                <div
              ...
              <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <OverviewItem icon={FiTag} title="Category" value={service.subcategory_name || "Finance"} />
        ...
        <div className="lg:col-span-1">
          <div className="lg:sticky lg:top-20 space-y-6">
            ...
            <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
              <div className="flex items-center mb-4">
                <div className="w-16 h-16 bg-blue-100 rounded-full flex items-center justify-center border-2 border-blue-500">
                  <span className="text-blue-700 font-bold text-xl">{getInitials(service.business_name)}</span>
                </div>
                <div className="ml-4">
                  <p className="font-bold text-lg text-gray-800">{service.business_name}</p>
                  {service.subcategory_name && (
                    <p className="text-xs text-blue-600">{service.subcategory_name}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                {service.contact_no && (
                  <a
                    ...
                    className="w-full bg-blue-600 text-white py-2.5 rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 font-medium transition"
                  >
                    <FiPhone /> Call Now
                  </a>
                )}
            ...
            <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <FiUser className="text-blue-600" /> Inquire Now
              </h2>
              ...
              <form onSubmit={handleInquirySubmit} className="space-y-4">
                ...
                <textarea
                  name="message"
                  placeholder="I am interested in your financial services. Please contact me for more details."
                  rows="3"
                  value={inquiryData.message}
                  onChange={(e) => setInquiryData({ ...inquiryData, message: e.target.value })}
                  className="w-full border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition resize-none"
                  disabled={submitting}
                />
                <button
                  type="submit"
                  disabled={submitting}
                  className={`w-full ${submitting ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'} text-white py-3 rounded-lg font-semibold transition`}
                >
            ...
            {/* Why Choose Us */}
            <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
              <h3 className="text-xl font-bold mb-4">Why Choose Our Financial Services?</h3>
              <ul className="text-sm space-y-3">
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
                  <span>Certified & Experienced Financial Experts</span>
                </li>
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
                  <span>Personalized Investment & Wealth Strategies</span>
                </li>
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
                  <span>Complete Tax Planning & Compliance Support</span>
                </li>
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
                  <span>Transparent & Competitive Fee Structure</span>
                </li>
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
