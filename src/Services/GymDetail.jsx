// src/pages/public/GymDetail.jsx
...
import { FaWhatsapp, FaStar, FaMapMarkerAlt, FaDumbbell } from "react-icons/fa";
import { publicAxios } from "../api/axios";
import MobileGymDetail from "./mobile/MobileGymDetail";
...
export default function GymDetail() {
  ...
  useEffect(() => {
    publicAxios.get('/api/services/service-subcategories/by_service/?service=Gym')
      .then(res => {
        if (res.data && res.data["Gym"]) {
          setSubcategories(res.data["Gym"]);
        }
      })
  ...
  useEffect(() => {
    const fetchServiceDetail = async () => {
      try {
        setLoading(true);
        const response = await publicAxios.get(`/api/public/gym-service/${id}/`);
        if (response.data) {
        ...
        console.error("Error fetching gym service:", err);
  ...
  const getSubcategoryName = (subcategoryId) => {
    if (!subcategoryId) return "Gym Service";
    ...
    return match?.subcategory_name || "Gym Service";
  };
  ...
  const handleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem("gym_favorites") || "[]");
  ...
  const OverviewItem = ({ icon: Icon, title, value }) => (
    <div className="bg-white p-3 rounded-lg shadow-md border border-gray-200 flex items-center space-x-3">
      <div className="pr-3 border-r border-orange-200">
    ...
    if (isMobile) {
    return <MobileGymDetail />;
  }

  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <FiLoader className="animate-spin text-4xl text-orange-600" />
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center h-screen">
        ...
        <button onClick={() => navigate("/gymhome")} className="bg-orange-600 text-white px-6 py-2 rounded-lg hover:bg-orange-700">
          Back to Gyms
        </button>
      </div>
    );
  ...
  const mainImage = service.main_image || "https://placehold.co/600x400/F97316/white?text=Gym+Service";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">{service.business_name || "Gym Service"}</h1>
          {service.subcategory_name && (
            <span className="inline-block mt-1 bg-orange-100 text-orange-800 text-xs px-3 py-1 rounded-full font-medium">
              {service.subcategory_name}
            </span>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center text-sm text-gray-500 mb-6 border-b pb-4 gap-4">
        <div className="flex items-center">
          <FiMapPin className="w-4 h-4 mr-1 text-orange-500" />
        ...
        <div className="lg:col-span-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
            <div className="relative col-span-2 row-span-2">
              <img className="rounded-lg shadow-md cursor-pointer hover:opacity-90 w-full h-96 object-cover transition duration-300"
                src={mainImage} alt={service.business_name || "Gym Service"}
                onClick={() => { setPhotoIndex(0); setIsOpen(true); }} />
              {slides.length > 0 && (
                <button onClick={() => setIsOpen(true)}
                  className="absolute bottom-4 right-4 bg-orange-600 text-white font-medium py-2 px-4 rounded-full hover:bg-orange-700 transition duration-300 flex items-center justify-center text-sm shadow-lg">
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <OverviewItem icon={FiTag} title="Category" value={service.subcategory_name || "Gym"} />
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <FaDumbbell className="text-orange-500 w-4 h-4 flex-shrink-0" />
                      ...
                      {formatCurrency(item.price) && (
                        <div className="ml-4 flex-shrink-0">
                          <span className="text-orange-600 font-bold text-lg">{formatCurrency(item.price)}</span>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
              <div className="bg-white p-3 rounded shadow-sm"><strong className="block text-xs text-gray-500 uppercase">City</strong>{service.city || "—"}</div>
              <div className="bg-white p-3 rounded shadow-sm"><strong className="block text-xs text-gray-500 uppercase">State</strong>{service.state || "—"}</div>
              <div className="bg-white p-3 rounded shadow-sm"><strong className="block text-xs text-gray-500 uppercase">Country</strong>{service.country || "—"}</div>
              <div className="bg-white p-3 rounded shadow-sm col-span-2"><strong className="block text-xs text-gray-500 uppercase">Full Address</strong>{service.address || "—"}</div>
              {service.location && (
                <div className="bg-white p-3 rounded shadow-sm col-span-2">
                  ...
                  <a href={service.location} target="_blank" rel="noopener noreferrer" className="text-orange-600 underline text-sm break-all">{service.location}</a>
                </div>
              )}
            </div>
          </section>
        ...
        <div className="lg:col-span-1">
          <div className="lg:sticky lg:top-20 space-y-8">
            ...
            <div className="bg-white shadow-lg rounded-lg p-6 border border-gray-100">
              <h2 className="text-xl font-bold mb-4 text-gray-700">Contact Gym</h2>
              <div className="flex items-center mb-4">
                <div className="w-16 h-16 bg-orange-100 rounded-full flex items-center justify-center flex-shrink-0 border-2 border-orange-500">
                  <span className="text-orange-700 font-bold text-xl">{getInitials(service.business_name)}</span>
                </div>
                <div className="ml-4">
                  <p className="font-bold text-lg text-gray-900">{service.business_name}</p>
                  {service.contact_no && (
                    <p className="text-sm text-gray-600 flex items-center gap-2 mt-1">
                      <FiPhone className="w-4 h-4 text-orange-500" /> {service.contact_no}
                    </p>
                  )}
                </div>
              </div>
              <div className="space-y-3 mt-4">
                {service.contact_no && (
                  <a href={`tel:${service.contact_no}`}
                    className="w-full bg-orange-600 text-white py-2 rounded-md hover:bg-orange-700 transition flex items-center justify-center gap-2 font-medium">
                    <FiPhone /> Call Now
                  </a>
                )}
            ...
            <div className="bg-white shadow-lg rounded-lg p-6 border border-gray-100">
              ...
              <form onSubmit={handleInquirySubmit} className="space-y-4">
                <input type="text" name="name" placeholder="Your Name*" required value={inquiryData.name}
                  ...
                  className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm focus:ring-orange-500 focus:border-orange-500" disabled={submitting} />
                ...
                <input type="text" name="city" placeholder="City*" required value={inquiryData.city}
                  ...
                  className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm focus:ring-orange-500 focus:border-orange-500" disabled={submitting} />
                <textarea name="message" placeholder="I am interested in your gym services." rows="3" value={inquiryData.message}
                  onChange={e => setInquiryData(p => ({...p, message: e.target.value}))}
                  className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm focus:ring-orange-500 focus:border-orange-500" disabled={submitting}></textarea>
                <button type="submit" disabled={submitting}
                  className={`w-full ${submitting ? 'bg-orange-400' : 'bg-orange-600 hover:bg-orange-700'} text-white py-3 rounded-md transition font-bold tracking-wide flex items-center justify-center`}>
            ...
            <div className="bg-white shadow-lg rounded-lg p-6 border border-gray-100">
              ...
              <ul className="text-sm text-gray-700 space-y-3">
                <li className="flex items-center"><FiCheckCircle className="text-green-500 mr-3 w-5 h-5 flex-shrink-0" /> Certified Trainers</li>
                <li className="flex items-center"><FiCheckCircle className="text-green-500 mr-3 w-5 h-5 flex-shrink-0" /> Modern Equipment</li>
                <li className="flex items-center"><FiCheckCircle className="text-green-500 mr-3 w-5 h-5 flex-shrink-0" /> Clean Environment</li>
                <li className="flex items-center"><FiCheckCircle className="text-green-500 mr-3 w-5 h-5 flex-shrink-0" /> Flexible Memberships</li>
              </ul>
            </div>
          </div>
        </div>
