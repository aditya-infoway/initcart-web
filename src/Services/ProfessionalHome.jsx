// src/pages/home/ProfessionalHome.tsx
...
import { FaQuoteLeft, FaStar, FaFileInvoiceDollar, FaBalanceScale, FaHardHat } from "react-icons/fa";
import { publicAxios } from "../api/axios";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import MobileProfessionalHome from "./mobile/MobileProfessionalHome";
...
export default function ProfessionalHome() {
  ...
  const navigateToServiceDetail = (id) => {
    navigate(`/professionaldetail/${id}`);
  };

  const navigateToViewAll = (subcategory) => {
    if (!subcategory) return;
    navigate(`/searchservice?subcategory=${encodeURIComponent(subcategory)}&type=professional`);
  };
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      try {
        const response = await publicAxios.get("api/service-subcategories/by_service/?service=Professional");
        const data = response.data;
        let subs = [];
        if (data["Professional"]) subs = data["Professional"];
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        const professionalCities = cityData.filter(city => city.type === "professional_service");
        
        const filteredCities = selectedSubcategory
          ? professionalCities.filter(city =>
  ...
  useEffect(() => {
    const fetchServices = async () => {
      setLoading(true);
      try {
        const response = await publicAxios.get("api/public/professional-services/");
    ...
    const fetchAds = async () => {
      try {
        const [bigAdRes, smallAdsRes] = await Promise.all([
          publicAxios.get("api/init-professional-bigad/").catch(() => ({ data: null })),
          publicAxios.get("api/init-professional-smallads/").catch(() => ({ data: [] }))
        ]);
  ...
  const renderServiceBlocks = () => {
    const blocks = [];
    // Split into chunks of 2 for alternating with banners
                  ...
                  <div key={service.id} className="p-2">
                    <div className="bg-white rounded-xl overflow-hidden cursor-pointer shadow-md hover:shadow-xl transition-all duration-300 group h-full flex flex-col">
                      ...
                      <div className="relative h-48 w-full overflow-hidden">
                        <img
                          src={service.main_image || "https://placehold.co/400x300"}
                      ...
                      <div className="p-4 flex flex-col flex-grow">
                        ...
                        <p className="text-xs text-indigo-600 mb-2">{service.subcategory_name}</p>
                        ...
                        <div className="flex gap-2 mt-auto">
                          ...
                          <button
                            ...
                            className="flex-1 text-center bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-2 rounded-lg transition"
                          >
                            Inquiry
                          </button>
                        </div>
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent flex items-center p-8">
                <div className="text-white max-w-md">
                  ...
                  <button className="bg-indigo-600 hover:bg-indigo-700 px-6 py-2 rounded-lg text-sm font-semibold transition mt-2">
                    Learn More →
                  </button>
                </div>
              </div>
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <section className="relative w-full h-[560px]">
        <img
          src={ads.bigAd?.image || "https://images.pexels.com/photos/4307872/pexels-photo-4307872.jpeg?auto=compress&cs=tinysrgb&w=1600"}
        ...
        <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4">
          <h2 className="text-white text-3xl sm:text-4xl md:text-5xl font-bold mb-3 leading-tight">
            Find Trusted Professionals
          </h2>
          <p className="text-indigo-100 text-sm sm:text-base md:text-lg mb-8 max-w-2xl">
            Connect with verified Chartered Accountants, Lawyers, Architects, and more
          </p>

          {/* Category Tabs */}
          <div className="bg-black/50 backdrop-blur-sm text-white rounded-2xl flex items-center justify-start sm:justify-center gap-4 sm:gap-6 px-4 py-3 w-[95%] sm:w-[85%] md:w-[75%] lg:w-[65%] overflow-x-auto scrollbar-hide">
          ...
          {/* Search Bar */}
          <div className="relative bg-white shadow-xl flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-[95%] sm:w-[85%] md:w-[75%] lg:w-[65%] p-2 mt-[-1px] rounded-b-sm sm:rounded-full">
            {/* City Dropdown */}
              ...
              {showCities && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white shadow-lg rounded-lg z-30 max-h-60 overflow-y-auto border">
                  {cities.map(city => (
                    <div
                      key={city.id}
                      className="px-4 py-2 text-left hover:bg-indigo-50 cursor-pointer text-gray-700 transition"
                      onClick={() => {
            ...
            <div className="flex-1 w-full">
              <input
                type="text"
                placeholder="Search for CA, Lawyer, Architect, or professional service..."
                value={searchKeyword}
            ...
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-md sm:rounded-full font-semibold flex items-center justify-center gap-2 transition w-full sm:w-auto"
            >
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <section className="relative z-20 -mt-16">
          <div className="relative bg-indigo-50 rounded-tl-[80px] rounded-tr-[80px] py-16">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 px-4">
              {[
                {
                  icon: <FaFileInvoiceDollar className="text-4xl text-indigo-600" />,
                  title: "Financial Expertise (CA)",
                  text: "Comprehensive tax planning, audit, and business advisory services.",
                },
                {
                  icon: <FaBalanceScale className="text-4xl text-green-600" />,
                  title: "Legal Representation (Advocate)",
                  text: "Skilled lawyers for litigation, corporate law, and personal legal matters.",
                },
                {
                  icon: <FaHardHat className="text-4xl text-pink-600" />,
                  title: "Design & Build (Architect)",
                  text: "Innovative design and project management for residential and commercial spaces.",
                },
              ].map((item, i) => (
                <div
                  key={i}
          ...
          <div className="absolute -bottom-1 left-0 w-full overflow-hidden leading-[0]">
            <svg className="relative block w-full h-16" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" viewBox="0 0 1200 120">
              <path d="M0,0 C300,120 900,120 1200,0 L1200,120 L0,120 Z" fill="#eef2ff"></path>
            </svg>
          </div>
        </section>
          ...
          <>
                      ...
                      <div className="bg-white rounded-xl overflow-hidden cursor-pointer shadow-md hover:shadow-xl transition-all duration-300 group h-full flex flex-col">
                        ...
                        <div className="p-4 flex flex-col flex-grow">
                          ...
                          <div className="flex gap-2 mt-auto">
                            ...
                            <button onClick={() => openInquiryModal(service)} className="flex-1 text-center bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold py-2 rounded-lg transition">Inquiry</button>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {blockTwo.map((category, idx) => (
              <section key={idx} className="py-10">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-2xl font-semibold text-gray-800">{category.subcategory}</h2>
                  <button onClick={() => navigateToViewAll(category.subcategory)} className="text-indigo-600 hover:text-indigo-800 font-medium flex items-center gap-1">View All →</button>
                </div>
                <Slider {...categorySlider}>
                  {category.services?.map((service) => (
                    <div key={service.id} className="p-2">
                      <div className="bg-white rounded-xl overflow-hidden cursor-pointer shadow-md hover:shadow-xl transition-all duration-300 group h-full flex flex-col">
                        <div className="relative h-48 w-full overflow-hidden">
                          <img src={service.main_image || "https://placehold.co/400x300"} alt={service.business_name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" onClick={() => navigateToServiceDetail(service.id)} />
                          {service.status === "approved" && (
                            <div className="absolute top-3 right-3">
              ...
              <section className="py-6">
                <div className="relative rounded-xl overflow-hidden shadow-lg group cursor-pointer h-72" onClick={() => window.open(ads.bigAd.url, "_blank")}>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-transparent flex items-center p-8">
                    <div className="text-white max-w-md">
                      <h2 className="text-2xl md:text-3xl font-bold mb-2">{ads.bigAd.title}</h2>
                      <button className="bg-indigo-600 hover:bg-indigo-700 px-6 py-2 rounded-lg text-sm font-semibold transition mt-2">Learn More →</button>
                    </div>
                  </div>
                </div>
              </section>
      ...
      {showInquiryModal && selectedService && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeInquiryModal}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="bg-indigo-600 text-white text-lg font-bold px-6 py-4 flex justify-between items-center sticky top-0">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="name" required value={form.name} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">Phone Number *</label>
        <input type="tel" name="phone" required value={form.phone} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">Email (Optional)</label>
        <input type="email" name="email" value={form.email} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">City</label>
        <input type="text" name="city" value={form.city} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">Message</label>
        <textarea name="message" rows="3" value={form.message} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition" />
      </div>
      
      <button type="submit" disabled={submitting} className={`w-full ${submitting ? 'bg-indigo-400' : 'bg-indigo-600 hover:bg-indigo-700'} text-white py-3 rounded-lg font-semibold transition`}>
