// src/Services/ProfessionalDetail.jsx
...
import { FaWhatsapp, FaStar, FaMapMarkerAlt, FaUserTie, FaBuilding } from "react-icons/fa";
import { publicAxios } from "../api/axios";
import MobileProfessionalDetail from "./mobile/MobileProfessinalDetail";
...
export default function ProfessionalDetail() {
  ...
  useEffect(() => {
    const fetchServiceDetail = async () => {
      try {
        setLoading(true);
        const response = await publicAxios.get(`/api/public/professional-service/${id}/`);
        if (response.data) {
  ...
  const handleFavorite = () => {
    const favs = JSON.parse(localStorage.getItem("professional_favorites") || "[]");
    if (service && !favs.includes(service.id)) {
      favs.push(service.id);
      localStorage.setItem("professional_favorites", JSON.stringify(favs));
  ...
  const OverviewItem = ({ icon: Icon, title, value }) => (
    <div className="bg-white p-3 rounded-lg shadow-md border border-gray-100 flex items-center space-x-3">
      <div className="pr-3 border-r border-indigo-200">
    ...
    if (isMobile) {
    return <MobileProfessionalDetail />;
  }
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center h-screen">
        ...
        <span className="ml-3 text-lg text-gray-600">Loading professional details...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col justify-center items-center h-screen">
        ...
        <button onClick={() => navigate("/professionalhome")} className="bg-indigo-600 text-white px-6 py-2 rounded-lg hover:bg-indigo-700">
  ...
  // Prepare images
  const mainImage = service.main_image || "https://placehold.co/800x500/6366F1/white?text=Professional+Service";
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">{service.business_name || "Professional Service"}</h1>
          {service.subcategory_name && (
            <span className="inline-block mt-2 bg-indigo-100 text-indigo-800 text-xs px-3 py-1 rounded-full font-medium">
              {service.subcategory_name}
            </span>
          )}
        </div>
        <div className="flex items-center space-x-4 mt-3 md:mt-0">
          ...
          <button onClick={handleShare} className="text-gray-400 hover:text-indigo-600 transition">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          {/* Image Gallery - Left side big image + thumbnails below */}
          <div className="mb-8">
            {/* Main Large Image - Takes full width */}
            <div className="relative mb-3">
              ...
              {allImages.length > 0 && (
                <button
                  ...
                  className="absolute bottom-4 right-4 bg-indigo-600 text-white font-medium py-2 px-4 rounded-full hover:bg-indigo-700 flex items-center text-sm shadow-lg"
                >
              ...
              <div className="grid grid-cols-4 md:grid-cols-5 gap-2">
                ...
                {additionalImages.length > 5 && (
                  <div
                    ...
                    onClick={() => setIsOpen(true)}
                  >
                    <span className="text-indigo-600 font-medium">+{additionalImages.length - 5} more</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="space-y-6">
              ...
              <section className="p-6 bg-gray-50 rounded-xl">
                <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                  <FaUserTie className="text-indigo-600" /> About {service.business_name}
                </h2>
                <div
              ...
              <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <OverviewItem icon={FiTag} title="Category" value={service.subcategory_name || "Professional"} />
              ...
              <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white p-3 rounded-lg shadow-sm border border-gray-100">
                    ...
                    {/* ✅ REVIEWS SECTION - SABSE NEECHE ADD KARO */}
              ...
              {service.location && (
                <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                  ...
                  <a
                    ...
                    className="text-indigo-600 underline text-sm break-all hover:text-indigo-800"
                  >
                    {service.location}
                  </a>
                </div>
          ...
          <div className="lg:sticky lg:top-20 space-y-6">
            ...
            <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
              <div className="flex items-center mb-4">
                <div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center border-2 border-indigo-500">
                  <span className="text-indigo-700 font-bold text-xl">{getInitials(service.business_name)}</span>
                </div>
                <div className="ml-4">
                  <p className="font-bold text-lg text-gray-800">{service.business_name}</p>
                  {service.subcategory_name && (
                    <p className="text-xs text-indigo-600">{service.subcategory_name}</p>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-3">
                {service.contact_no && (
                  <a
                    ...
                    className="w-full bg-indigo-600 text-white py-2.5 rounded-lg hover:bg-indigo-700 flex items-center justify-center gap-2 font-medium transition"
                  >
                    <FiPhone /> Call Now
                  </a>
                )}
            ...
            <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <FiUser className="text-indigo-600" /> Inquire Now
              </h2>
              ...
              <form onSubmit={handleInquirySubmit} className="space-y-4">
                ...
                <textarea
                  name="message"
                  placeholder="I am interested in your professional services. Please contact me for more details."
                  rows="3"
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className={`w-full ${submitting ? 'bg-indigo-400' : 'bg-indigo-600 hover:bg-indigo-700'} text-white py-3 rounded-lg font-semibold transition`}
                >
            ...
            <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100">
              ...
              <ul className="text-sm space-y-3">
                ...
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
                  <span>Timely & Reliable Service Delivery</span>
                </li>
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
                  <span>Confidential & Secure Consultations</span>
                </li>
                <li className="flex items-start gap-2">
                  <FiCheckCircle className="text-green-500 mt-0.5 w-4 h-4 flex-shrink-0" />
