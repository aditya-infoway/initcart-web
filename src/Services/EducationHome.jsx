// src/pages/home/EducationHome.tsx
...
import { FaQuoteLeft, FaStar, FaGraduationCap, FaChalkboardTeacher, FaBook, FaLaptopCode, FaSchool } from "react-icons/fa";
import { publicAxios } from "../api/axios";
import "slick-carousel/slick/slick.css";
import "slick-carousel/slick/slick-theme.css";
import MobileEducationHome from "./mobile/MobileEducationHome";
...
export default function EducationHome() {
  ...
  const navigateToServiceDetail = (id) => {
    navigate(`/educationdetail/${id}`);
  };

  const navigateToViewAll = (subcategory) => {
    if (!subcategory) return;
    navigate(`/searchservice?subcategory=${encodeURIComponent(subcategory)}&type=education`);
  };

  const navigateToServiceList = () => {
    const params = new URLSearchParams();
    params.append("type", "education");
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      try {
        const response = await publicAxios.get("api/service-subcategories/by_service/?service=Education");
        const data = response.data;
        let subs = [];
        if (data["Education"]) subs = data["Education"];
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        const educationCities = cityData.filter(city => city.type === "education_service");
        
        const filteredCities = selectedSubcategory
          ? educationCities.filter(city =>
  ...
  useEffect(() => {
    const fetchServices = async () => {
      setLoading(true);
      try {
        const response = await publicAxios.get("api/public/education-services/");
    ...
    const fetchAds = async () => {
      try {
        const [bigAdRes, smallAdsRes] = await Promise.all([
          publicAxios.get("api/init-education-bigad/").catch(() => ({ data: null })),
          publicAxios.get("api/init-education-smallads/").catch(() => ({ data: [] }))
        ]);
        ...
        console.error("Error fetching education ads:", error);
      }
    };
    fetchAds();
  }, []);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <section className="relative w-full bg-gradient-to-br from-purple-700 via-purple-600 to-indigo-700">
        <div className="absolute inset-0 bg-[url('https://images.pexels.com/photos/256417/pexels-photo-256417.jpeg?auto=compress&cs=tinysrgb&w=1600')] opacity-10 bg-cover bg-center"></div>
        ...
        <div className="relative py-16 md:py-20 px-4">
          <div className="max-w-6xl mx-auto text-center">
            <h2 className="text-white text-3xl sm:text-4xl md:text-5xl font-bold mb-4 leading-tight">
              Find Best Educational Institutes & Courses
            </h2>
            <p className="text-purple-100 text-sm sm:text-base md:text-lg mb-8 max-w-2xl mx-auto">
              Connect with verified Schools, Colleges, Coaching Centers, and Training Institutes
            </p>
            ...
            <div className="mt-6 bg-white shadow-xl flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full max-w-4xl mx-auto p-2 rounded-xl sm:rounded-full">
                ...
                {showCities && (
                  <div className="absolute top-full left-0 mt-1 w-full bg-white shadow-lg rounded-lg z-30 max-h-60 overflow-y-auto border">
                    {cities.map(city => (
                      <div
                        key={city.id}
                        className="px-4 py-2 text-left hover:bg-purple-50 cursor-pointer text-gray-700 transition"
                        onClick={() => {
              ...
              <div className="flex-1 w-full">
                <input
                  type="text"
                  placeholder="Search for School, College, Coaching Center, Training Institute..."
                  value={searchKeyword}
              ...
              <button
                ...
                className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-md sm:rounded-full font-semibold flex items-center justify-center gap-2 transition w-full sm:w-auto"
              >
      ...
      <div className="max-w-6xl mx-auto px-4">
        <section className="relative z-20 -mt-16">
          <div className="relative bg-purple-50 rounded-tl-[80px] rounded-tr-[80px] py-16">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 px-4">
              {[
                {
                  icon: <FaSchool className="text-4xl text-purple-600" />,
                  title: "Top Schools & Colleges",
                  text: "Find the best educational institutions with excellent infrastructure and faculty.",
                },
                {
                  icon: <FaChalkboardTeacher className="text-4xl text-blue-600" />,
                  title: "Expert Coaching Centers",
                  text: "Coaching for competitive exams, board exams, and skill development.",
                },
                {
                  icon: <FaLaptopCode className="text-4xl text-green-600" />,
                  title: "Professional Training",
                  text: "IT training, language courses, and professional certification programs.",
                },
              ].map((item, i) => (
                <div
                  key={i}
          ...
          <div className="absolute -bottom-1 left-0 w-full overflow-hidden leading-[0]">
            <svg className="relative block w-full h-16" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" viewBox="0 0 1200 120">
              <path d="M0,0 C300,120 900,120 1200,0 L1200,120 L0,120 Z" fill="#faf5ff"></path>
            </svg>
          </div>
        </section>
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500">Loading educational services...</p>
          </div>
        ) : (
          <>
            {blockOne.map((category, idx) => (
              <section key={idx} className="py-10">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => navigateToViewAll(category.subcategory)} className="text-purple-600 hover:text-purple-800 font-medium flex items-center gap-1">View All →</button>
                </div>
                    ...
                    <div key={service.id} className="p-2">
                      <div className="bg-white rounded-xl overflow-hidden cursor-pointer shadow-md hover:shadow-xl transition-all duration-300 group h-full flex flex-col">
                        <div className="relative h-48 w-full overflow-hidden">
                          <img src={service.main_image || "https://placehold.co/400x300/7c3aed/white?text=Education+Service"} alt={service.business_name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" onClick={() => navigateToServiceDetail(service.id)} />
                        ...
                        <div className="p-4 flex flex-col flex-grow">
                          ...
                          <p className="text-xs text-purple-600 mb-2">{service.subcategory_name}</p>
                          ...
                          <div className="flex gap-2 mt-auto">
                            ...
                            <button onClick={() => openInquiryModal(service)} className="flex-1 text-center bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold py-2 rounded-lg transition">Inquiry</button>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="relative rounded-xl overflow-hidden shadow-lg group cursor-pointer h-72" onClick={() => window.open(ads.bigAd.url, "_blank")}>
                  ...
                  <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-transparent flex items-center p-8">
                    <div className="text-white max-w-md">
                      ...
                      <button className="bg-purple-600 hover:bg-purple-700 px-6 py-2 rounded-lg text-sm font-semibold transition mt-2">Learn More →</button>
                    </div>
                  </div>
                </div>
              </section>
      ...
      {showInquiryModal && selectedService && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeInquiryModal}>
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="bg-purple-600 text-white text-lg font-bold px-6 py-4 flex justify-between items-center sticky top-0">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="name" required value={form.name} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">Phone Number *</label>
        <input type="tel" name="phone" required value={form.phone} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">Email (Optional)</label>
        <input type="email" name="email" value={form.email} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">City</label>
        <input type="text" name="city" value={form.city} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition" />
      </div>
      
      <div>
        <label className="block text-gray-700 font-medium mb-1">Message</label>
        <textarea name="message" rows="3" value={form.message} onChange={handleChange} className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition" />
      </div>
      
      <button type="submit" disabled={submitting} className={`w-full ${submitting ? 'bg-purple-400' : 'bg-purple-600 hover:bg-purple-700'} text-white py-3 rounded-lg font-semibold transition`}>
