...
import MobileTravelAgencyHome from "./mobile/MobileTravelAgencyHome";
...
export default function TravelAgencyHome() {
  ...
  if (isMobile) {
    return <MobileTravelAgencyHome />;
  }

  // ✅ FIXED: Add type=travel_agency parameter
  const navigateToServiceList = () => {
    const params = new URLSearchParams();
    params.append("type", "travel_agency"); // 👈 ADD THIS
  ...
  // ✅ FIXED: Fetch subcategories from correct API
  ...
  // ✅ FIXED: Fetch from public travel agency list endpoint
              ...
              <div className="bg-white border-2 border-blue-500 rounded-2xl shadow-md hover:shadow-xl transition-transform transform hover:-translate-y-1 cursor-pointer w-full flex flex-col overflow-hidden">
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex gap-2 mt-4">
                    ...
                    <button onClick={() => openInquiryModal(service)} className="flex-1 text-center bg-blue-600 text-white font-medium py-2 rounded-md hover:bg-blue-700 transition">Inquiry</button>
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="bg-gray-50">
      ...
      <section className="relative w-full h-[540px]">
        <img src="https://images.pexels.com/photos/3889885/pexels-photo-3889885.jpeg?auto=compress&cs=tinysrgb&w=1600" alt="travel-banner" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-blue-900/40"></div>
        <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-4">
          <h2 className="text-white text-2xl sm:text-3xl md:text-4xl font-bold mb-2 leading-snug">Find the Best Travel Agencies and Tour Services Near You</h2>
          <p className="text-blue-100 text-xs sm:text-sm md:text-base mb-6 max-w-[90%] sm:max-w-[80%]">
            <span className="font-semibold">300+</span> verified travel agencies and <span className="font-semibold">150+</span> tour services!
          </p>
          <div className="bg-[#00000080] text-white rounded-2xl flex items-center justify-start sm:justify-center gap-4 sm:gap-6 px-4 py-3 w-[95%] sm:w-[85%] md:w-[70%] lg:w-[60%] overflow-x-auto scrollbar-hide min-h-[40px]">
            ...
            {subcategories.map((tab, idx) => (
              <button key={idx}
                className={`flex-shrink-0 pb-1 px-2 text-xs sm:text-sm md:text-base font-semibold border-b-2 ${selectedSubcategory === tab ? "border-white text-white" : "border-transparent text-gray-300 hover:text-white hover:border-white"} transition`}
          ...
          <div className="relative bg-white shadow-lg flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-[95%] sm:w-[85%] md:w-[70%] lg:w-[60%] p-2 mt-[-1px] rounded-b-sm sm:rounded-full">
            <div className="relative sm:w-40 w-full sm:border-r border-b sm:border-b-0">
              ...
              {showCities && (
                <div className="absolute top-full left-0 mt-1 w-full bg-white shadow-lg rounded-lg z-30">
                  {cities.map(city => (
                    <div key={city.id} className="px-4 py-2 text-left hover:bg-gray-100 cursor-pointer text-gray-700"
                      onClick={() => { setSelectedCity(city.name); setShowCities(false); }}>
                      {city.name}
                    </div>
                  ))}
                </div>
            ...
            <div className="flex-1 w-full">
              <input type="text" placeholder="Search for travel agency or tour service" value={searchKeyword}
            ...
            <button onClick={navigateToServiceList} className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-md sm:rounded-full font-semibold flex items-center justify-center transition w-full sm:w-auto">
        ...
        <div className="relative bg-blue-100 rounded-tl-[80px] rounded-tr-[80px] py-16">
          <div className="max-w-6xl mx-auto px-4">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {[
                { icon: <FaPlane className="text-4xl text-blue-600" />, title: "Global Destinations", text: "Access to the best flights, hotels, and experiences worldwide.", bg: "bg-white" },
                { icon: <FaCalendarCheck className="text-4xl text-green-600" />, title: "Flexible Booking", text: "Easily modify or cancel plans with transparent policies.", bg: "bg-white" },
                { icon: <FaRoute className="text-4xl text-pink-600" />, title: "Personalized Itineraries", text: "Expert agents craft a journey tailored to your interests.", bg: "bg-white" },
              ].map((item, i) => (
      ...
      {showInquiryModal && selectedService && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50" onClick={closeInquiryModal}>
          <div className="bg-white rounded-xl shadow-xl w-11/12 md:w-3/4 lg:w-1/2 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <InquiryForm serviceData={selectedService} />
            </div>
          </div>
        </div>
      )}
...
const InquiryForm = ({ serviceData, prefillMessage }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!vendorId || !category) { setErrorMsg("Vendor or Service info missing."); return; }
    setSubmitting(true);
    try {
      ...
      if (response.status === 201) { setSuccessMsg("✅ Inquiry submitted successfully!"); setForm({ name: "", phone: "", email: "", city: "", message: "" }); }
    } catch (err) { setErrorMsg("❌ Failed to submit inquiry."); } finally { setSubmitting(false); }
  };
  ...
  return (
    <div className="bg-white rounded-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={submitting} className={`w-full ${submitting ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'} text-white py-3 rounded-md font-semibold`}>{submitting ? 'Submitting...' : 'Send Inquiry'}</button>
      </form>
    </div>
  );
};